...
enum {
    ...
    PSS_TILEMAP_CONTEST,
    PSS_TILEMAP_BG,
    PSS_TILEMAP_HEADER_SIDEBAR,
    PSS_TILEMAP_INFO_EGG,
    PSS_TILEMAP_INFO_RENTAL,
    PSS_TILEMAP_STATS_IVS_EVS,    
    PSS_TILEMAP_STATS_BASE,
    PSS_TILEMAP_BATTLE_DETAIL,
    PSS_TILEMAP_BATTLE_FORGET,
    PSS_TILEMAP_CONTEST_DETAIL,
    PSS_TILEMAP_CONTEST_FORGET,
    PSS_TILEMAP_COUNT,
};
...
#define PSS_LABEL_WINDOW_PROMPT_EVIV 5
#define PSS_LABEL_WINDOW_PROMPT_BASE 6
#define PSS_LABEL_WINDOW_PROMPT_STATS 7
#define PSS_LABEL_WINDOW_PROMPT_INFO 8 // Move Info
#define PSS_LABEL_WINDOW_PROMPT_SWITCH 9 // Switch Moves
#define PSS_LABEL_WINDOW_UNUSED1 10

// Info screen
#define PSS_LABEL_WINDOW_POKEMON_INFO_OTID 11
#define PSS_LABEL_WINDOW_POKEMON_INFO_OT_NAME 12
#define PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES 13
#define PSS_LABEL_WINDOW_POKEMON_INFO_TYPE 14
#define PSS_LABEL_WINDOW_POKEMON_INFO_RENTAL 15

// Skills screen
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_TOP 16 // HP
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM 17 // Attack, Defense, Sp. Attack, Sp. Defense, Speed
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_NATURE 18
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP 19 // EXP, Next Level
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS 20

// Moves screen
#define PSS_LABEL_WINDOW_MOVES_POWER_ACC 21 // Also contains the power and accuracy values
#define PSS_LABEL_WINDOW_MOVES_APPEAL_JAM 22

// Above/below the pokemon's portrait (left)
#define PSS_LABEL_WINDOW_PORTRAIT_LEVEL 23
#define PSS_LABEL_WINDOW_PORTRAIT_NICKNAME 24
#define PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_LABEL 25
#define PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME 26
#define PSS_LABEL_WINDOW_END 27

// Dynamic fields for the Pokémon Info page
#define PSS_DATA_WINDOW_INFO_OTID 0
#define PSS_DATA_WINDOW_INFO_OT_NAME 1
#define PSS_DATA_WINDOW_INFO_SPECIES 2
#define PSS_DATA_WINDOW_INFO_EXP_AND_NEXT_LEVEL 3
#define PSS_DATA_WINDOW_INFO_MEMO 4

// Dynamic fields for the Pokémon Skills page
#define PSS_DATA_WINDOW_SKILLS_STATS_HP 0
#define PSS_DATA_WINDOW_SKILLS_STATS_LEFT 1 // (None), IVs, Base IVs
#define PSS_DATA_WINDOW_SKILLS_STATS_RIGHT 2 // Stats, EVs, Base Stats
#define PSS_DATA_WINDOW_SKILLS_NATURE 3
...
#define PSS_DATA_WINDOW_MOVE_DESC 2
#define PSS_DATA_WINDOW_ABILITY_NAME_AND_DESC 3
...
enum
{
    ...
    SPRITE_ARR_ID_MOVE_SELECTOR1 = SPRITE_ARR_ID_TYPE + TYPE_ICON_SPRITE_COUNT, // sprites that make up the selector
    SPRITE_ARR_ID_MOVE_SELECTOR2 = SPRITE_ARR_ID_MOVE_SELECTOR1 + MOVE_SELECTOR_SPRITES_COUNT,
    SPRITE_ARR_ID_COUNT = SPRITE_ARR_ID_MOVE_SELECTOR2 + MOVE_SELECTOR_SPRITES_COUNT
};

#define TILE_EMPTY_APPEAL_HEART  0x3022
#define TILE_FILLED_APPEAL_HEART 0x3021
#define TILE_FILLED_JAM_HEART    0x3028
#define TILE_EMPTY_JAM_HEART     0x3029
...
static EWRAM_DATA struct PokemonSummaryScreenData
{
    ...
    u16 bgTilemapBuffers[PSS_TILEMAP_COUNT][2][0x400];
    u8 mode;
    bool8 isBoxMon;
    u8 curMonIndex;
    u8 maxMonIndex;
...
EWRAM_DATA u8 gLastViewedMonIndex_New = 0;
static EWRAM_DATA u8 sMoveSlotToReplace_New = 0;
...
static void SwitchToStatsView(void);
static void SwitchToIVsEVsView(void);
static void SwitchToBaseStatsView(void);
...
static void PrintMonAbilityNameAndDesc(void);
...
static void PrintLabels_AtkDefSpAtkSpDefSpeed(void);
static void BufferCurrentHPStat(void);
static void PrintCurrentHPStat(void);
static void BufferCurrentStats(void);
static void PrintCurrentStats(void);
static void BufferIVs(void);
static void PrintIVs(void);
static void BufferEVs(void);
static void PrintEVs(void);
static u8 GetNatureModTextColor(s8, u8);
static void PrintNature(void);
static const u8 *GetNatureModArrowText(s8);
static void PrintExpPointsNextLevel(void);
static void PrintBattleMoves(void);
static void Task_PrintBattleMoves(u8);
static void PrintMoveNameAndPP(u8);
static void PrintMovePowerAndAccuracy(u16);
...
static void ClearNewMoveDetailsOrCancelText(void);
...
static const struct WindowTemplate sSummaryTemplate[] =
{
    ...
    [PSS_LABEL_WINDOW_POKEMON_INFO_OTID] = {
        .bg = 0,
        .tilemapLeft = 12,
        .tilemapTop = 2,
        .width = 4,
    ...
    [PSS_LABEL_WINDOW_POKEMON_INFO_OT_NAME] = {
        .bg = 0,
        .tilemapLeft = 12,
        .tilemapTop = 4,
        .width = 2,
    ...
    [PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES] = {
        .bg = 0,
        .tilemapLeft = 12,
        .tilemapTop = 6,
        .width = 5,
    ...
    [PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_TOP] = {
        .bg = 0,
        .tilemapLeft = 13,
        .tilemapTop = 3,
        .width = 2,
    ...
    [PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM] = {
        .bg = 0,
        .tilemapLeft = 11,
        .tilemapTop = 6,
        .width = 8,
    ...
    [PSS_LABEL_WINDOW_POKEMON_SKILLS_NATURE] = {
        .bg = 0,
        .tilemapLeft = 13,
        .tilemapTop = 17,
        .width = 6,
    ...
    [PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME] = {
        .bg = 0,
        .tilemapLeft = 0,
        .tilemapTop = 18,
        .width = 11,
...
static const struct WindowTemplate sPageInfoTemplate[] =
{
    ...
    [PSS_DATA_WINDOW_INFO_EXP_AND_NEXT_LEVEL] = {
        .bg = 0,
        .tilemapLeft = 13,
        .tilemapTop = 11,
        .width = 16,
...
static const struct WindowTemplate sPageMovesTemplate[] = // This is used for both battle and contest moves
{
    ...
    [PSS_DATA_WINDOW_ABILITY_NAME_AND_DESC] = {
        .bg = 0,
        .tilemapLeft = 12,
        .tilemapTop = 13,
        .width = 17,
...
static const u8 sHPPPLayout[] = _("{DYNAMIC 0}/{DYNAMIC 1}");
...
static u8 ShowCategoryIcon(u32 category)
{
    if (sMonSummaryScreen->categoryIconSpriteId == 0xFF)
        sMonSummaryScreen->categoryIconSpriteId = CreateSprite(&sSpriteTemplate_CategoryIcons, 71, 121, 0);
...
void ShowPokemonSummaryScreen_New(u8 mode, void *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void))
{
    ...
    switch (mode)
    {
    case NEW_SUMMARY_MODE_NORMAL:
    case NEW_SUMMARY_MODE_BOX:
        sMonSummaryScreen->minPageIndex = 0;
        sMonSummaryScreen->maxPageIndex = PSS_PAGE_COUNT - 1;
        break;
    case NEW_SUMMARY_MODE_LOCK_MOVES:
        sMonSummaryScreen->minPageIndex = 0;
        sMonSummaryScreen->maxPageIndex = PSS_PAGE_COUNT - 1;
        sMonSummaryScreen->lockMovesFlag = TRUE;
        break;
    case NEW_SUMMARY_MODE_SELECT_MOVE:
    ...
    sMonSummaryScreen->categoryIconSpriteId = 0xFF;
    sMonSummaryScreen->viewingIvEv = FALSE;
    SummaryScreen_SetAnimDelayTaskId_New(TASK_NONE);
...
{
    ShowPokemonSummaryScreen_New(NEW_SUMMARY_MODE_SELECT_MOVE, mons, monIndex, maxMonIndex, callback);
...
{
    ShowPokemonSummaryScreen_New(mode, mons, monIndex, maxMonIndex, callback);
...
static void InitBGs(void)
{
    ...
    SetBgTilemapBuffer(1, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_INFO][0]);
    SetBgTilemapBuffer(2, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_HEADER_SIDEBAR][0]);
    SetBgTilemapBuffer(3, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_BG][0]);
    ResetAllBgsCoordinates();
    ScheduleBgCopyTilemapToVram(1);
    ScheduleBgCopyTilemapToVram(2);
    ScheduleBgCopyTilemapToVram(3);
    ChangeBgX(1, 0x10000, BG_COORD_SET); // Set info page to be on screen at start
...
static bool8 DecompressGraphics(void)
{
    ...
    case 0:
        ...
        DecompressAndCopyTileDataToVram(1, &gNewSummaryScreen_Gfx, 0, 0, 0);
        sMonSummaryScreen->switchCounter++;
        break;
    case 1:
        if (FreeTempTileDataBuffersIfPossible() != 1)
        {
            LZDecompressWram(gNewSummaryPage_BG_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_BG][0]);
            LZDecompressWram(gNewSummaryPage_BG_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_BG][1]);
            sMonSummaryScreen->switchCounter++;
        }
        break;
    case 2:
        LZDecompressWram(gNewSummaryPage_Header_Side_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_HEADER_SIDEBAR][0]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 3:
        LZDecompressWram(gNewSummaryPage_Info_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_INFO][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 4:
        LZDecompressWram(gNewSummaryPage_InfoEgg_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_INFO_EGG][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 5:
        LZDecompressWram(gNewSummaryPage_Skills_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_INFO_RENTAL][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 6:
        LZDecompressWram(gNewSummaryPage_Skills_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_STATS][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 7:
        LZDecompressWram(gNewSummaryPage_Skills_IVsEVs_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_STATS_IVS_EVS][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 8:
        LZDecompressWram(gNewSummaryPage_BattleMoves_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_BATTLE][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 9:
        LZDecompressWram(gNewSummaryPage_ContestMoves_Tilemap, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_CONTEST][1]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 10:
        LoadCompressedPalette(gNewSummaryScreen_Pal, BG_PLTT_ID(0), 8 * PLTT_SIZE_4BPP);
...
static bool8 ExtractMonDataToSummaryStruct(struct Pokemon *mon)
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (sMonSummaryScreen->monList.mons == gPlayerParty || sMonSummaryScreen->mode == NEW_SUMMARY_MODE_BOX || sMonSummaryScreen->handleDeoxys == TRUE)
        {
...
static void SetDefaultTilemaps(void)
{
    ...
    else
    {
        ...
        // ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES);
...
static void CloseSummaryScreen(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        gLastViewedMonIndex_New = sMonSummaryScreen->curMonIndex;
...
static void Task_HandleInput(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(A_BUTTON))
        {
            ...
            {
                PlaySE(SE_SELECT);
                if (sMonSummaryScreen->viewingIvEv)
                    SwitchToStatsView();
                else
                    SwitchToIVsEVsView();
            }
...
#define scrollState data[0] 
#define bgId        data[1] 
#define direction   data[2] 
...
static void ChangePage(u8 taskId, s8 delta)
{
    ...
    if (summary->isEgg)
        return;
    else if (direction == -1 && sMonSummaryScreen->currPageIndex == sMonSummaryScreen->minPageIndex)
        return;
    else if (direction == 1 && sMonSummaryScreen->currPageIndex == sMonSummaryScreen->maxPageIndex)
        return;

    PlaySE(SE_SELECT);
    ClearPageWindowTilemaps(sMonSummaryScreen->currPageIndex);
    sMonSummaryScreen->currPageIndex += direction;
    scrollState = 0;
    SetTaskFuncWithFollowupFunc(taskId, PssScrollCurrentPageOut, gTasks[taskId].func);
    CreateTextPrinterTask(sMonSummaryScreen->currPageIndex);
    sMonSummaryScreen->viewingIvEv = FALSE; // Always start on regular stats view
    HidePageSpecificSprites();
}

static void PssScrollCurrentPageOut(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    bgId = 1;
    
    ChangeBgX(bgId, PAGE_SCROLL_SPEED * 256, BG_COORD_SUB);
    scrollState += PAGE_SCROLL_SPEED;
    if (scrollState > 0xFF)
    {        
        scrollState = 0;
        ScheduleBgCopyTilemapToVram(1);
        SetBgTilemapBuffer(bgId, sMonSummaryScreen->bgTilemapBuffers[sMonSummaryScreen->currPageIndex][0]);
        ShowBg(1);
        ShowBg(2);
        gTasks[taskId].func = PssScrollNewPageIn;
    }
}

static void PssScrollNewPageIn(u8 taskId)
{
    s16 *data = gTasks[taskId].data;

    ChangeBgX(bgId, PAGE_SCROLL_SPEED * 256, BG_COORD_ADD);
    scrollState += PAGE_SCROLL_SPEED;
    if (scrollState > 0xFF)
    {
        scrollState = 0;
...
static void SwitchToStatsView(void)
{
    // Clear previous stat text
    RemoveWindowByIndex(PSS_DATA_WINDOW_SKILLS_STATS_HP);
    ...
    // Update tiles
    ScheduleBgCopyTilemapToVram(1);
    SetBgTilemapBuffer(1, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_STATS][0]);
    ShowBg(1);
    ShowBg(2);
    ...
    sMonSummaryScreen->viewingIvEv = FALSE;
    PutPageWindowTilemaps(sMonSummaryScreen->currPageIndex); // Updates the EVs/IVs/Stats prompt
}
...
static void SwitchToIVsEVsView(void)
{
    ...
    // Update tiles
    ScheduleBgCopyTilemapToVram(1);
    SetBgTilemapBuffer(1, sMonSummaryScreen->bgTilemapBuffers[PSS_TILEMAP_STATS_IVS_EVS][0]);
    // ShowBg(1);
    // ShowBg(2);
    ...
    sMonSummaryScreen->viewingIvEv = TRUE;
    PutPageWindowTilemaps(sMonSummaryScreen->currPageIndex); // Updates the EVs/IVs/Stats prompt
}

static void SwitchToBaseStatsView(void)
{
    // Hide previous stat text 
    ClearWindowTilemap(PSS_LABEL_WINDOW_PROMPT_BASE);
    ClearWindowTilemap(PSS_DATA_WINDOW_SKILLS_STATS_HP);
...
static void SwitchToMoveSelection(u8 taskId)
{
    ...
    ClearWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME);
...
static void ChangeSelectedMove(s16 *taskData, s8 direction, u8 *moveIndexPtr)
{
        ...
        && sMonSummaryScreen->newMove == MOVE_NONE)
    {
        ...
        DestroyCategoryIcon();
        ScheduleBgCopyTilemapToVram(0);
        //FillWindowPixelRect(PSS_LABEL_WINDOW_MOVES_POWER_ACC, PIXEL_FILL(0), 53, 0, 19, 48);
...
static void CloseMoveSelectMode(u8 taskId)
{
    ...
    ClearNewMoveDetailsOrCancelText();
    ...
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME);
    ScheduleBgCopyTilemapToVram(0);
    ScheduleBgCopyTilemapToVram(1);
    ScheduleBgCopyTilemapToVram(2);
    gTasks[taskId].func = Task_HandleInput;
        ...
        if (gPaletteFade.active != TRUE)
        {
            ...
            else if (JOY_NEW(A_BUTTON))
            {
                if (CanReplaceMove() == TRUE)
                {
                    StopPokemonAnimations();
                    PlaySE(SE_SELECT);
                    sMoveSlotToReplace_New = sMonSummaryScreen->firstMoveIndex;
                    gSpecialVar_0x8005 = sMoveSlotToReplace_New;
                    BeginCloseSummaryScreen(taskId);
                }
                else
                {
            ...
            else if (JOY_NEW(B_BUTTON))
            {
                StopPokemonAnimations();
                PlaySE(SE_SELECT);
                sMoveSlotToReplace_New = MAX_MON_MOVES;
...
static void Task_ShowPowerAccWindow(u8 taskId)
{
    ...
    if (data[1] <= 0 || data[1] >= sBattleMoveTilemapCtrl.field_6)
    {
        ...
        else
        {
            ...
            // PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES);
            PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_LABEL);
            PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME);
        }
        ScheduleBgCopyTilemapToVram(0);
...
{
    AddTextPrinterParameterized3(windowId, fontId, x, y, sTextColors[colorId], 0, string);
}
...
static void PrintMonInfo(void)
{
    ...
    FillWindowPixelBuffer(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME, PIXEL_FILL(0));
    // FillWindowPixelBuffer(PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES, PIXEL_FILL(0));
    if (!sMonSummaryScreen->summary.isEgg)
        PrintNotEggInfo();
    else
        PrintEggInfo();
...
static void PrintNotEggInfo(void)
{
    ...
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_LEVEL, FONT_SHORT, gStringVar1, 0, 1, 0, 0);
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_LEVEL);

    // Nickname & Gender
    GetMonNickname(mon, gStringVar1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, FONT_SHORT, gStringVar1, 2, 1, 0, 0);
    PrintGenderSymbol(mon, summary->species2);
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME);

    // Held Item
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_LABEL, FONT_SMALL, gText_HeldItem, 0, 2, 0, 0);
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_LABEL);    
    PrintHeldItemName();
}
...
static void PrintEggInfo(void)
{
    ...
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, FONT_SHORT, gStringVar1, 2, 1, 0, 1);
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME);
...
static void PrintGenderSymbol(struct Pokemon *mon, u16 species)
{
    if (species != SPECIES_NIDORAN_M && species != SPECIES_NIDORAN_F)
    {
        ...
        {
        case MON_MALE:
            PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, FONT_SHORT, gText_MaleSymbol, 74, 1, 0, 3);
            break;
        case MON_FEMALE:
            PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, FONT_SHORT, gText_FemaleSymbol, 74, 1, 0, 4);
            break;
        }
    }
}
...
static void PrintPageNamesAndStats(void)
{
    ...
    // Page Titles
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_TITLE, FONT_SHORT, gText_PkmnInfo, 2, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_TITLE, FONT_SHORT, gText_PkmnSkills, 2, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_BATTLE_MOVES_TITLE, FONT_SHORT, gText_BattleMoves, 2, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_CONTEST_MOVES_TITLE, FONT_SHORT, gText_ContestMoves, 2, 1, 0, 1);

    // Controls
    stringXPos = GetStringRightAlignXOffset(FONT_SMALL, gText_Controls_PageCancel, 84);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_CANCEL, FONT_SMALL, gText_Controls_PageCancel, stringXPos, 1, 0, 0);
    stringXPos = GetStringRightAlignXOffset(FONT_SMALL, gText_Controls_PageIVsEVs, 84);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_EVIV, FONT_SMALL, gText_Controls_PageIVsEVs, stringXPos, 1, 0, 0);
    stringXPos = GetStringRightAlignXOffset(FONT_SMALL, gText_Controls_PageIVsEVs, 84);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_STATS, FONT_SMALL, gText_Controls_PageStats, stringXPos, 1, 0, 0);
    stringXPos = GetStringRightAlignXOffset(FONT_SMALL, gText_Controls_PageDetail, 84);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_INFO, FONT_SMALL, gText_Controls_PageDetail, stringXPos, 1, 0, 0);
    stringXPos = GetStringRightAlignXOffset(FONT_SMALL, gText_Controls_PickSwitch, 84);
    PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_SWITCH, FONT_SMALL, gText_Controls_PickSwitch, stringXPos, 1, 0, 0);

    // Info Page
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_RENTAL, FONT_SHORT, gText_RentalPkmn, 0, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_OTID, FONT_SHORT, gText_IDNumber, 0, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_OT_NAME, FONT_SHORT, gText_OT, 0, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES, FONT_SHORT, gText_Species, 0, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_TYPE, FONT_SHORT, gText_Type, 0, 1, 0, 1);

    // Stats Page
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_TOP, FONT_SHORT, gText_HP4, 2, 1, 0, 1);
    PrintLabels_AtkDefSpAtkSpDefSpeed(); // Incuding nature mod colors
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_NATURE, FONT_SHORT, gText_Nature, 2, 1, 0, 1);

    // Moves Pages
    PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, FONT_SHORT, gText_Category, 0, 3, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, FONT_SHORT, gText_Power, 0, 17, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, FONT_SHORT, gText_Accuracy2, 0, 31, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_APPEAL_JAM, FONT_SHORT, gText_Appeal, 0, 1, 0, 1);
    PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_APPEAL_JAM, FONT_SHORT, gText_Jam, 0, 17, 0, 1);
}
...
static void PutPageWindowTilemaps(u8 page)
{
    ...
    case PSS_PAGE_INFO:
        ...
        PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_OTID);
        PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_OT_NAME);
        PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES);
    ...
    case PSS_PAGE_SKILLS:
        ...
        PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_TOP);
        PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM);
        PutWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_NATURE);
...
static void ClearPageWindowTilemaps(u8 page)
{
    ...
    case PSS_PAGE_INFO:
        ...
        ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_OTID);
        ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_OT_NAME);
        ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_INFO_SPECIES);
    ...
    case PSS_PAGE_SKILLS:
        ...
        ClearWindowTilemap(PSS_LABEL_WINDOW_PROMPT_BASE);
        ClearWindowTilemap(PSS_LABEL_WINDOW_PROMPT_STATS);
        ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_TOP);
        ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM);
        ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_NATURE);
        break;
    case PSS_PAGE_BATTLE_MOVES:
        if (sMonSummaryScreen->mode == NEW_SUMMARY_MODE_SELECT_MOVE)
        {
...
static void PrintMonOTName(void)
{
    ...
    if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE)
    {
        windowId = AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_OT_NAME);
        // PrintTextOnWindow(windowId, FONT_SHORT, gText_OTSlash, 0, 1, 0, 1);
        // x = GetStringWidth(FONT_SHORT, gText_OTSlash, 0);
        if (sMonSummaryScreen->summary.OTGender == 0)
            PrintTextOnWindow(windowId, FONT_SHORT, sMonSummaryScreen->summary.OTName, 0, 1, 0, 5);
        else
            PrintTextOnWindow(windowId, FONT_SHORT, sMonSummaryScreen->summary.OTName, 0, 1, 0, 6);
    }
}
...
static void PrintMonOTID(void)
{
    ...
    if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE)
    {
        ...
        PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_OTID), FONT_SHORT, gStringVar1, /*xPos*/ 0, 1, 0, 0);
    }
}
...
static void PrintMonSpecies(void)
{
    ...
    StringCopy(gStringVar1, &GetSpeciesName(sum->species2)[0]);
    windowId = AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_SPECIES);
    PrintTextOnWindow(windowId, FONT_SHORT, gStringVar1, 0, 1, 0, 0);
}

static void PrintMonAbilityNameAndDesc(void)
{
    u8 windowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_ABILITY_NAME_AND_DESC);
    int xPos;
    u16 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum);

    xPos = GetStringCenterAlignXOffset(FONT_SHORT, gAbilitiesInfo[ability].name, 80);
    PrintTextOnWindow(windowId, FONT_SMALL, gAbilitiesInfo[ability].name, 48 + xPos, 5, 0, 1);
    PrintTextOnWindow(windowId, FONT_SMALL, gAbilitiesInfo[ability].description, 0, 17, 0, 0);
...
// {
//     u16 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species, sMonSummaryScreen->summary.abilityNum);
//     PrintTextOnWindow(AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_ABILITY_NAME_AND_DESC), FONT_SMALL, gAbilitiesInfo[ability].description, 0, 17, 0, 0);
// }
...
static void PrintMonTrainerMemo(void)
{
    PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_MEMO), FONT_SMALL, gStringVar4, 0, 1, 0, 0);
}
...
static void PrintEggOTName(void)
{
    u32 windowId = AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_OT_NAME);
    u32 width = GetStringWidth(FONT_SHORT, gText_OTSlash, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, gText_OTSlash, 0, 1, 0, 1);
    PrintTextOnWindow(windowId, FONT_SHORT, gText_FiveMarks, width, 1, 0, 1);
}
...
static void PrintEggOTID(void)
{
    ...
    PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_OTID), FONT_SHORT, gStringVar1, x, 1, 0, 1);
}
...
static void PrintEggState(void)
{
    ...
    PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_MOVE_DESC), FONT_SHORT, text, 0, 1, 0, 0);
}
...
static void PrintEggMemo(void)
{
    ...
    PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_MEMO), FONT_SMALL, text, 0, 1, 0, 0);
}
...
static void PrintSkillsPageText(void)
{
    ...
    // PrintIVs();
    //BufferCurrentStats();
    PrintLabels_AtkDefSpAtkSpDefSpeed();
    if (sMonSummaryScreen->viewingIvEv)
    {
        PrintIVs();
        PrintEVs();
...
static void Task_PrintSkillsPage(u8 taskId)
{
    ...
    case 6:
        // PrintCurrentStats();
        PrintLabels_AtkDefSpAtkSpDefSpeed(); // Updates dynamic nature colors
        break;
    case 7:
        PrintCurrentStats();
        PrintNature();
...
static void PrintHeldItemName(void)
{
    ...
    x = GetStringCenterAlignXOffset(FONT_SMALL, text, 72) + 6;
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME, FONT_SMALL, text, x, 2, 0, 0);
    PutWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_HELD_ITEM_NAME);
}
...
static void PrintRibbonCount(void)
{
    ...
    PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_STATS_HP), FONT_SHORT, text, x, 1, 0, 0);
}
...
static void PrintLabels_AtkDefSpAtkSpDefSpeed(void)
{
    ...
    FillWindowPixelBuffer(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, PIXEL_FILL(0));

    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, gText_Attack3, 18, 1, 0, GetNatureModTextColor(natureMod[STAT_ATK - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, gText_Defense3, 18, 17, 0, GetNatureModTextColor(natureMod[STAT_DEF - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, gText_SpAtk4, 18, 33, 0, GetNatureModTextColor(natureMod[STAT_SPATK - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, gText_SpDef4, 18, 49, 0, GetNatureModTextColor(natureMod[STAT_SPDEF - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, gText_Speed2, 18, 65, 0, GetNatureModTextColor(natureMod[STAT_SPEED - 1], 1));  

    // Nature Mod Arrows
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, GetNatureModArrowText(natureMod[STAT_ATK - 1]), 6, 2, 0, GetNatureModTextColor(natureMod[STAT_ATK - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, GetNatureModArrowText(natureMod[STAT_DEF - 1]), 6, 18, 0, GetNatureModTextColor(natureMod[STAT_DEF - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, GetNatureModArrowText(natureMod[STAT_SPATK - 1]), 6, 34, 0, GetNatureModTextColor(natureMod[STAT_SPATK - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, GetNatureModArrowText(natureMod[STAT_SPDEF - 1]), 6, 50, 0, GetNatureModTextColor(natureMod[STAT_SPDEF - 1], 1));
    PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_BOTTOM, FONT_SHORT, GetNatureModArrowText(natureMod[STAT_SPEED - 1]), 6, 66, 0, GetNatureModTextColor(natureMod[STAT_SPEED - 1], 1));

    // PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP, FONT_SHORT, gText_ExpPoints, 6, 1, 0, 1);
    // PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP, FONT_SHORT, gText_NextLv, 6, 17, 0, 1);
    // PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS, FONT_SHORT, gText_Status, 2, 1, 0, 1);
}
...
static void BufferCurrentHPStat(void)
{
    ...
    // TODO: Handle 4 digits for Dynamax HP
    ...
    DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sHPPPLayout);
...
{
    int x;
    const u8 *mintNature = gNatureNamePointers[sMonSummaryScreen->summary.mintNature];

    x = GetStringCenterAlignXOffset(FONT_SHORT, gStringVar4, 64);
    PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_STATS_HP), FONT_SHORT, gStringVar4, x, 1, 0, 0);
    // PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_NATURE), FONT_SHORT, gStringVar4, x, 1, 0, 0);

    // x = GetStringCenterAlignXOffset(FONT_SHORT, mintNature, 48);
    // PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_NATURE), FONT_SHORT, mintNature, x, 1, 0, 0);
}
...
static void BufferCurrentStats(void)
{
    ...
    DynamicPlaceholderTextUtil_Reset();
    BufferStat(attackString, natureMod[STAT_ATK - 1], sMonSummaryScreen->summary.atk, 0, 3);
    BufferStat(defenseString, natureMod[STAT_DEF - 1], sMonSummaryScreen->summary.def, 1, 3);
    BufferStat(spAtkString, natureMod[STAT_SPATK - 1], sMonSummaryScreen->summary.spatk, 2, 3);
    BufferStat(spDefString, natureMod[STAT_SPDEF - 1], sMonSummaryScreen->summary.spdef, 3, 3);
    BufferStat(speedString, natureMod[STAT_SPEED - 1], sMonSummaryScreen->summary.speed, 4, 3);
...
static void PrintCurrentStats(void)
{
    // PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_STATS_RIGHT), FONT_SHORT, gStringVar4, 0, 1, 0, 0);
    ...
    // Convert stat numbers to aligned string
    ...
    PrintTextOnWindow(windowId, FONT_SHORT, attackString, 3,  1, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, defenseString, 3, 17, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, spAtkString, 3, 33, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, spDefString, 3, 49, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, speedString, 3, 65, 0, 0);
...
static void BufferIVs(void)
{
    ...
    DynamicPlaceholderTextUtil_Reset();
    BufferStat(hpString, natureMod[STAT_HP - 1], sMonSummaryScreen->summary.HPIV, 0, 3);
    BufferStat(attackString, natureMod[STAT_ATK - 1], sMonSummaryScreen->summary.atkIV, 0, 3);
    BufferStat(defenseString, natureMod[STAT_DEF - 1], sMonSummaryScreen->summary.defIV, 1, 3);
    BufferStat(spAtkString, natureMod[STAT_SPATK - 1], sMonSummaryScreen->summary.spatkIV, 2, 3);
    BufferStat(spDefString, natureMod[STAT_SPDEF - 1], sMonSummaryScreen->summary.spdefIV, 3, 3);
    BufferStat(speedString, natureMod[STAT_SPEED - 1], sMonSummaryScreen->summary.speedIV, 4, 3);
    DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sStatsRightColumnLayout);
...
static void PrintIVs(void)
{
    ...
    // Convert stat numbers to aligned string
    ConvertIntToDecimalStringN(hpString, sMonSummaryScreen->summary.HPIV, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(attackString, sMonSummaryScreen->summary.atkIV, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(defenseString, sMonSummaryScreen->summary.defIV, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(spAtkString, sMonSummaryScreen->summary.spatkIV, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(spDefString, sMonSummaryScreen->summary.spdefIV, STR_CONV_MODE_RIGHT_ALIGN, 2);
    ConvertIntToDecimalStringN(speedString, sMonSummaryScreen->summary.speedIV, STR_CONV_MODE_RIGHT_ALIGN, 2);

    PrintTextOnWindow(hpWindowId, FONT_SHORT, hpString, 3,  1, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, attackString, 3,  1, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, defenseString, 3, 17, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, spAtkString, 3, 33, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, spDefString, 3, 49, 0, 0);
...
static void BufferEVs(void)
{
    ...
    DynamicPlaceholderTextUtil_Reset();
    BufferStat(hpString, natureMod[STAT_HP - 1], sMonSummaryScreen->summary.HPEV, 0, 3);
    BufferStat(attackString, natureMod[STAT_ATK - 1], sMonSummaryScreen->summary.atkEV, 0, 3);
    BufferStat(defenseString, natureMod[STAT_DEF - 1], sMonSummaryScreen->summary.defEV, 1, 3);
    BufferStat(spAtkString, natureMod[STAT_SPATK - 1], sMonSummaryScreen->summary.spatkEV, 2, 3);
    BufferStat(spDefString, natureMod[STAT_SPDEF - 1], sMonSummaryScreen->summary.spdefEV, 3, 3);
    BufferStat(speedString, natureMod[STAT_SPEED - 1], sMonSummaryScreen->summary.speedEV, 4, 3);
    DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sStatsRightColumnLayout);
...
static void PrintEVs(void)
{
    ...
    ConvertIntToDecimalStringN(attackString, sMonSummaryScreen->summary.atkEV, STR_CONV_MODE_RIGHT_ALIGN, 3);
    ConvertIntToDecimalStringN(defenseString, sMonSummaryScreen->summary.defEV, STR_CONV_MODE_RIGHT_ALIGN, 3);
    ConvertIntToDecimalStringN(spAtkString, sMonSummaryScreen->summary.spatkEV, STR_CONV_MODE_RIGHT_ALIGN, 3);
    ConvertIntToDecimalStringN(spDefString, sMonSummaryScreen->summary.spdefEV, STR_CONV_MODE_RIGHT_ALIGN, 3);
    ConvertIntToDecimalStringN(speedString, sMonSummaryScreen->summary.speedEV, STR_CONV_MODE_RIGHT_ALIGN, 3);

    PrintTextOnWindow(hpWindowId, FONT_SHORT, hpString, 43,  1, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, attackString, 19,  1, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, defenseString, 19, 17, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, spAtkString, 19, 33, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, spDefString, 19, 49, 0, 0);
    PrintTextOnWindow(windowId, FONT_SHORT, speedString, 19, 65, 0, 0);
...
static u8 GetNatureModTextColor(s8 mod, u8 defaultColorId)
{
    u8 colorId;
...
static const u8 *GetNatureModArrowText(s8 mod)
{    
    if (mod > 0)
...
{
    struct PokeSummary *sum = &sMonSummaryScreen->summary;
    u8 windowId = AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_EXP_AND_NEXT_LEVEL);
    int x;
    u32 expToNextLevel;

    ConvertIntToDecimalStringN(gStringVar1, sum->exp, STR_CONV_MODE_RIGHT_ALIGN, 7);
    x = GetStringCenterAlignXOffset(FONT_SHORT, gStringVar1, 64);
    PrintTextOnWindow(windowId, FONT_SHORT, gStringVar1, x, 1, 0, 0);
    ...
    x = GetStringCenterAlignXOffset(FONT_SHORT, gStringVar1, 48);
    PrintTextOnWindow(windowId, FONT_SHORT, gStringVar1, 80 + x, 1, 0, 0);
}
...
static void PrintMoveNameAndPP(u8 moveIndex)
{
    ...
    if (move != 0)
    {
        ...
        PrintTextOnWindow(moveNameWindowId, FONT_SHORT, GetMoveName(move), 0, moveIndex * 16 + 1, 0, 1);
    ...
    {
        PrintTextOnWindow(moveNameWindowId, FONT_SHORT, gText_OneDash, 0, moveIndex * 16 + 1, 0, 1);
        text = gText_TwoDashes;
        ppState = 12;
        x = GetStringCenterAlignXOffset(FONT_SHORT, text, 44);
    }

    PrintTextOnWindow(ppValueWindowId, FONT_SMALL, text, x, moveIndex * 16 + 1, 0, ppState);
}
...
static void PrintMovePowerAndAccuracy(u16 moveIndex)
{
    ...
    if (moveIndex == 0)
    {
        ...
        PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, FONT_SHORT, text, 53, 17, 0, 0);
        PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, FONT_SHORT, text, 53, 31, 0, 0);
    }
    else
    {
        FillWindowPixelRect(PSS_LABEL_WINDOW_MOVES_POWER_ACC, PIXEL_FILL(0), 53, 0, 19, 48);
...
static void PrintContestMoveDescription(u8 moveSlot)
{
    ...
    if (move != MOVE_NONE)
    {
        u8 windowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESC);
        PrintTextOnWindow(windowId, FONT_SHORT, gContestEffectDescriptionPointers[gMovesInfo[move].contestEffect], 0, 1, 0, 1);
    }
}
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            if (moveEffect != EFFECT_PLACEHOLDER)
                PrintTextOnWindow(windowId, FONT_SHORT, gMovesInfo[move].description, 0, 1, 0, 1);
            else
                PrintTextOnWindow(windowId, FONT_SHORT, gNotDoneYetDescription, 0, 1, 0, 1);
        }
        else
        {
            PrintTextOnWindow(windowId, FONT_SHORT, gContestEffectDescriptionPointers[gMovesInfo[move].contestEffect], 0, 1, 0, 1);
...
static void PrintNewMoveDetailsOrCancelText(void)
{
    ...
    if (sMonSummaryScreen->newMove == MOVE_NONE)
    {
        PrintTextOnWindow(windowId1, FONT_SHORT, gText_Cancel, 0, 65, 0, 1);
    }
    else
    {
        ...
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            PrintTextOnWindow(windowId1, FONT_SHORT, GetMoveName(move), 0, 65, 0, 6);
        else
            PrintTextOnWindow(windowId1, FONT_SHORT, GetMoveName(move), 0, 65, 0, 5);
        ...
        DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sHPPPLayout);
        PrintTextOnWindow(windowId2, FONT_SMALL, gStringVar4, GetStringRightAlignXOffset(FONT_SMALL, gStringVar4, 44), 65, 0, 12);
    }
}
...
static void PrintHMMovesCantBeForgotten(void)
{
    u8 windowId = AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_DESC);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(0));
    PrintTextOnWindow(windowId, FONT_SHORT, gText_HMMovesCantBeForgotten2, 6, 1, 0, 1);
}
...
static void SetMonTypeIcons(void)
{
    ...
    if (summary->isEgg)
    {
        SetTypeSpritePosAndPal(TYPE_MYSTERY, 152, 64, SPRITE_ARR_ID_TYPE);
        SetSpriteInvisibility(SPRITE_ARR_ID_TYPE + 1, TRUE);
    }
    else
    {
        SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[0], 152, 64, SPRITE_ARR_ID_TYPE);
        if (gSpeciesInfo[summary->species].types[0] != gSpeciesInfo[summary->species].types[1])
        {
            SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[1], 184, 64, SPRITE_ARR_ID_TYPE + 1);
        ...
        {
            SetTypeSpritePosAndPal(summary->teraType, 216, 64, SPRITE_ARR_ID_TYPE + 2);
        }
    }
}
...
static void SetMoveTypeIcons(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            SetTypeSpritePosAndPal(gMovesInfo[summary->moves[i]].type, 93, 24 + (i * 16), i + SPRITE_ARR_ID_TYPE);
        }
        else
...
static void SetContestMoveTypeIcons(void)
{
    ...
    {
        if (summary->moves[i] != MOVE_NONE)
            SetTypeSpritePosAndPal(NUMBER_OF_MON_TYPES + gMovesInfo[summary->moves[i]].contestCategory, 93, 24 + (i * 16), i + SPRITE_ARR_ID_TYPE);
        else
            SetSpriteInvisibility(i + SPRITE_ARR_ID_TYPE, TRUE);
    }
}
...
static void SetNewMoveTypeIcon(void)
{
    ...
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            SetTypeSpritePosAndPal(gMovesInfo[sMonSummaryScreen->newMove].type, 93, 104, SPRITE_ARR_ID_TYPE + 4);
        else
            SetTypeSpritePosAndPal(NUMBER_OF_MON_TYPES + gMovesInfo[sMonSummaryScreen->newMove].contestCategory, 93, 104, SPRITE_ARR_ID_TYPE + 4);
    }
}
...
{
    struct PokeSummary *summary = &sMonSummaryScreen->summary;
    u8 spriteId = CreateSprite(&gMultiuseSpriteTemplate, 48, 84, 5);
...
static void CreateMonMarkingsSprite(struct Pokemon *mon)
{
    ...
    if (sprite != NULL)
    {
        ...
        sMonSummaryScreen->markingsSprite->x = 16;
        sMonSummaryScreen->markingsSprite->y = 124;
...
static void CreateCaughtBallSprite(struct Pokemon *mon)
{
    ...
    LoadBallGfx(ball);
    sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_BALL] = CreateSprite(&gBallSpriteTemplates[ball], 8, 38, 0);
...
static void CreateMoveSelectorSprites(u8 idArrayStart)
{
    ...
    if (sMonSummaryScreen->currPageIndex >= PSS_PAGE_BATTLE_MOVES)
    {
        ...
        {
            spriteIds[i] = CreateSprite(&sMoveSelectorSpriteTemplate, i * 16 + 97, 32, subpriority);
            if (i == 0)
                StartSpriteAnim(&gSprites[spriteIds[i]], 4); // left
            else if (i == MOVE_SELECTOR_SPRITES_COUNT - 1)
            {
                gSprites[spriteIds[i]].x -= 7;
...
static void CreateHeldItemSprite(void)
{
    ...
    *spriteId = AddItemIconSprite(TAG_HELD_ITEM, TAG_HELD_ITEM, sMonSummaryScreen->summary.item); 
    gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_ITEM]].oam.priority = 2;
    ...
    *spriteShadowId = AddItemIconSprite(TAG_HELD_ITEM, TAG_DROP_SHADOW, sMonSummaryScreen->summary.item); 
    gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_ITEM_SHADOW]].oam.priority = 3;
