...
static void Select_Task_OpenSummaryScreen(u8 taskId)
{
    ...
    case STATE_SUMMARY_SHOW:
        ...
        if (NEW_SUMMARY_SCREEN)
            ShowPokemonSummaryScreen_New(SUMMARY_MODE_LOCK_MOVES, sFactorySelectMons, currMonId, SELECTABLE_MONS_COUNT - 1, CB2_InitSelectScreen);
        else
...
static void Swap_Task_OpenSummaryScreen(u8 taskId)
{
    ...
    case STATE_SUMMARY_SHOW:
        ...
        if (NEW_SUMMARY_SCREEN)
            ShowPokemonSummaryScreen_New(SUMMARY_MODE_NORMAL, gPlayerParty, sFactorySwapScreen->cursorPos, FRONTIER_PARTY_SIZE - 1, CB2_InitSwapScreen);
        else
